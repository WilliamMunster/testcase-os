...
`case-review` skill は `_system/config.yaml` で定義されたレビュー次元を使用します。チェックする次元をカスタマイズできます：
...
レビューレポートは `reviews/{module}/` に生成され、wikilinks で元のケースにリンクされます。
...
| **case-review** | "テストケースをレビュー" | 多次元レビュー（完全性、正確性、追跡可能性、知識一貫性、リスク、準拠性）とレポート生成。 |
| **case-import** | "login.feature からケースをインポート" | Gherkin または Excel 形式を標準 Markdown カードに変換し、サニタイズを行います。 |
| **knowledge-import** | "この URL からナレッジをインポート" | 様々なソースからビジネスまたは技術ナレッジをインポートし、標準化されたナレッジカードを生成します。 |
